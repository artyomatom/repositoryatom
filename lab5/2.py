...
def count(lst):
    """
    Подсчитывает количество элементов в списке, включая вложенные списки.
    Сам вложенный список считается за 1 элемент + его содержимое.
    """
    total = 0
    for item in lst:
...
# Используем кэш для оптимизации рекурсии
...
if __name__ == "__main__":
    ...
    print("=== Тесты функции count ===")
    print(count([]))                    # 0
    print(count([1, 2, 3]))             # 3
    print(count(["x", "y", ["z"]]))     # 4
    print(count([1, 2, [3, 4, [5]]]))   # 7

    print("\n=== Тесты функции x_recursive ===")
    print(x_recursive(1))               # 1.0
    print(x_recursive(2))               # -0.125
    print(x_recursive(3))               # ~0.1667
    print(x_recursive(4))               # ~0.1042
    print(x_recursive(5))